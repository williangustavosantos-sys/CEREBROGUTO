...
// Regra fundadora vigente: o bônus do Pacto vale 100 no saldo geral/Evoluir,
// sem fingir presença nem inflar os placares semanais/mensais da Arena.

describe("Arena XP — bônus do Pacto fica fora dos períodos competitivos", () => {
  beforeEach(() => writeArenaStore({ profiles: {}, events: [] }));

  it("bônus do pacto credita apenas o saldo geral em 100", () => {
    ...
    assert.equal(r.weeklyXp, 0, "Semana não recebe o bônus do pacto");
    assert.equal(r.monthlyXp, 0, "Mês não recebe o bônus do pacto");
  ...
  it("bônus + validação: períodos recebem apenas a validação", () => {
    ...
    assert.equal(r.weeklyXp, 100, "weekly = somente validação");
    assert.equal(r.monthlyXp, 100, "monthly = somente validação");
  ...
  it("migração v3 remove o bônus dos períodos e é idempotente", () => {
    const now = new Date("2026-07-18T12:00:00.000Z");
    const store = {
      profiles: {
        legacy: {
          userId: "legacy", displayName: "Legacy", pairName: "GUTO & LEGACY", arenaGroupId: "g",
          avatarStage: "baby" as const, totalXp: 150, weeklyXp: 150, monthlyXp: 150,
