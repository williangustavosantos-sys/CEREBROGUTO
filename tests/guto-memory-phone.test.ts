...
const testMemoryFile = join(tmpDir, "guto-memory.phone-test.json");
...
describe("GutoMemory phone isolation", () => {
  before(async () => {
    ...
    if (!address || typeof address === "string") throw new Error("Failed to bind phone memory test server.");
  ...
  it("remove telefone legado da resposta e da persistência de GutoMemory", async () => {
    const userId = "memory-phone-legacy";
    writeRawMemory(userId, { phone: "+390212345678" });
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(readMemory(userId), "phone"), false);
  });

  it("ignora telefone enviado no payload público de /guto/memory", async () => {
    const userId = "memory-phone-post";
    writeRawMemory(userId, {});

    const res = await fetch(`${baseUrl}/guto/memory`, {
      method: "POST",
      headers: authHeaders(userId),
      body: JSON.stringify({ phone: "+551199999999", weightKg: 82 }),
    });
